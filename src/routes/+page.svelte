<script lang="ts">
  ...
  const getallConvo = async () => {
    const resultList = await pb.collection("conversation").getList(1, 50, {
      filter: 'user = "n8fmsajxxmm6yg0"',
    });
    conversations = resultList.items;
    conversations.forEach(async (element: any) => {
      await getMsg(element.id);
    });
  ...
  const addMsg = async () => {
    var msg = {
      conversationId: "32497dsdjhkjsdfg",
      message: {
        content: "Hello World!",
        type: "text",
        sender: { id: "n8fmsajxxmm6yg0" },
        createdAt: Date.now(),
        updatedAt: Date.now(),
        seenBy: "",
      },
    };
    const message = await pb.collection("message").create(msg);
  };
  ...
  const getMsg = async (conversation: string) => {
    const resultmessages = await pb.collection("message").getList(1, 50, {
      filter: `conversation = "${conversation}"`,
    });
    messages[conversation] = resultmessages.items;
    ...
    <div id="sidepanel">
      <div id="profile">
        <div class="wrap">
          ...
          <div id="expanded">
            <label for="twitter"
              ><i class="fa fa-facebook fa-fw" aria-hidden="true" /></label
            >
            <input name="twitter" type="text" value="mikeross" />
            <label for="twitter"
              ><i class="fa fa-twitter fa-fw" aria-hidden="true" /></label
            >
            <input name="twitter" type="text" value="ross81" />
            <label for="twitter"
              ><i class="fa fa-instagram fa-fw" aria-hidden="true" /></label
            >
      ...
      <div id="search">
        <label for=""><i class="fa fa-search" aria-hidden="true" /></label>
    ...
    <div class="content">
      ...
      <div class="message-input">
        <div class="wrap">
          ...
          <button class="submit"
            ><i class="fa fa-paper-plane" aria-hidden="true" /></button
          >
        </div>
      </div>
    </div>
